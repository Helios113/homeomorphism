"""Baseline Experiment 3: Syntactic Disintegration (Permutation).

Measures per-sublayer Jacobian properties AND multi-depth ID estimation when the
model uses syntactic disintegration: tokens are permuted while respecting padding.
...
Run layout:
  results/baseline_permutation/<run_id>/
    config.json
    manifest.json
    results.jsonl

CLI:
  python experiments/baseline_exp_3_permutation.py \\
...
def run(
    *,
    ...
    """Run syntactic disintegration baseline; return path to the run directory."""
    ...
    print(f"[baseline_permutation] run_dir = {run_dir}")
    ...
    print(
        f"[baseline_permutation] model={model_name} L={L} d={d_model} "
    ...
    with results_path.open("w") as f:
        ...
        if N >= 2:
            ...
            n_id_rows = 0
            print(f"[baseline_permutation] insufficient inputs for ID estimation (N={N} < 2)")
    ...
    print(
        f"\n[baseline_permutation] done: {n_jacobian_rows} jacobian rows, "
        f"{n_id_rows if N >= 2 else 0} id rows, {manifest['duration_sec']}s total"
    )
    print(f"[baseline_permutation] outputs in {run_dir}")
    return run_dir


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Baseline Exp 3: Syntactic Disintegration (Permutation)")
    ...
    p.add_argument("--output-root", type=Path, default=Path("results/baseline_permutation"))
    return p
